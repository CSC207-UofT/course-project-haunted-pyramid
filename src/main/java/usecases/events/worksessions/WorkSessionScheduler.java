...
public class WorkSessionScheduler implements EventListObserver {
    ...
    private void autoSchedule(UUID deadline, EventManager eventManager){
        Long totalHours = eventManager.getTotalHoursNeeded(deadline);
        ...
        while (totalHours > 0) {
            ...
            totalHours -= length;
            System.out.println("scheduled: " + eventManager.getWorkSessions(deadline).size());
        }
    }
    ...
    private void mergeSessions(UUID deadline, EventManager eventManager, LocalDateTime idealStartTime, Long length, Event newSession) {
        Event toMerge = sessionAdjacent(idealStartTime, length, deadline, eventManager);
        if (toMerge != null){
            eventManager.getWorkSessions(deadline).remove(newSession);
            eventManager.getWorkSessions(deadline).remove(toMerge);
    ...
    private Event sessionAdjacent(LocalDateTime time, Long length, UUID deadline, EventManager eventManager) {
    ...
    public static void main(String[] args){
        ...
        UUID deadline = eventManager.getID(eventManager.addEvent("deadline", LocalDateTime.now().plusDays(7)));
